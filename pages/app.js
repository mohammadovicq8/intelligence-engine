...
export default function App() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: bg, color: text, fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', transition: 'all 0.2s' }}>
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '1.25rem 1.5rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem', paddingBottom: '1rem', borderBottom: `1px solid ${border}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
            <h1 onClick={() => router.push('/')} style={{ fontSize: 22, fontWeight: 800, margin: 0, letterSpacing: '-0.5px', cursor: 'pointer', color: text }}>Verdict</h1>
            <div style={{ display: 'flex', gap: 2, background: surface2, borderRadius: 10, padding: 3 }}>
              {['warroom', 'vault'].map(t => (
                <button key={t} onClick={() => setTab(t)} style={{ padding: '6px 14px', background: tab === t ? (dm ? '#fff' : '#1a1a1a') : 'transparent', color: tab === t ? (dm ? '#000' : '#fff') : textMuted, border: 'none', borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <button onClick={signOut} style={{ fontSize: 11, color: textMuted, background: 'none', border: `1px solid ${border}`, borderRadius: 6, padding: '4px 10px', cursor: 'pointer' }}>Out</button>
          </div>
        </div>
          ...
          <>
            <div style={{ display: 'flex', gap: 8, marginBottom: 28 }}>
              <input
                ...
                style={{ flex: 1, padding: '14px 18px', fontSize: 15, border: `1.5px solid ${loading ? '#534AB7' : border}`, borderRadius: 12, outline: 'none', background: surface, color: text, transition: 'border 0.2s' }}
              />
              <button onClick={runAnalysis} disabled={loading} style={{ padding: '14px 28px', background: loading ? '#1e1b4b' : (dm ? '#fff' : '#1a1a1a'), color: loading ? '#818cf8' : (dm ? '#000' : '#fff'), border: 'none', borderRadius: 12, fontSize: 14, fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', minWidth: 160, transition: 'all 0.2s', letterSpacing: '-0.2px' }}>
            ...
            {loading && (
              <div style={{ marginBottom: 28 }}>
                <div style={{ fontSize: 12, color: textMuted, marginBottom: 16, textAlign: 'center', letterSpacing: '0.5px' }}>{stage}</div>
            ...
            {stockData && (
              <div style={{ background: surface, border: `1px solid ${border}`, borderRadius: 14, padding: '1.25rem', marginBottom: 24 }}>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, marginBottom: 12, flexWrap: 'wrap' }}>
                  <span style={{ fontSize: 20, fontWeight: 800, color: text }}>{stockData.name}</span>
                  <span style={{ fontSize: 13, color: textMuted, background: surface2, padding: '2px 8px', borderRadius: 6 }}>{stockData.symbol}</span>
                  <span style={{ fontSize: 20, fontWeight: 700, color: text }}>${stockData.price}</span>
                ...
                <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap', marginBottom: stockData.news?.length > 0 ? 12 : 0 }}>
                  {[
                    ...
                    <div key={m.label}>
                      ...
                      <div style={{ fontSize: 13, fontWeight: 600, color: text }}>{m.value}</div>
                    </div>
                  ))}
                </div>
            ...
            {round1 && (
              <div style={{ marginBottom: 24 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: 10 }}>
                  {Object.entries(round1).map(([id, text2]) => {
                    ...
                    return (
                      <div key={id} style={{ background: p.bg, border: `1px solid ${p.border}`, borderRadius: 12, padding: '1rem 1.25rem' }}>
                        ...
                        <div style={{ fontSize: 13, lineHeight: 1.75, color: '#cbd5e1' }}><ReactMarkdown>{text2}</ReactMarkdown></div>
                      </div>
                    );
                  })}
                </div>
            ...
            {round2 && (
              <div style={{ marginBottom: 24 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: 10 }}>
                  {Object.entries(round2).map(([id, text2]) => {
                    const p = personaConfig[id];
                    return (
                      <div key={id} style={{ background: surface, border: `1.5px solid ${p.border}`, borderRadius: 12, padding: '1rem 1.25rem' }}>
                        ...
                        <div style={{ fontSize: 13, lineHeight: 1.75, color: textMuted }}><ReactMarkdown>{text2}</ReactMarkdown></div>
                      </div>
                    );
                  })}
                </div>
            ...
            {synthesis && (
              <div style={{ background: surface, border: `1px solid ${border}`, borderRadius: 16, padding: '2rem', marginBottom: 24, position: 'relative', overflow: 'hidden' }}>
                  ...
                  <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                    {[
                      ...
                      <div key={c.label} style={{ background: surface2, border: `1px solid ${border}`, borderRadius: 10, padding: '10px 16px' }}>
                        ...
                        <div style={{ fontSize: 18, fontWeight: 700, color: text }}>{c.value}</div>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 10, marginBottom: 20 }}>
                  {[
                    { label: 'Key Risk', value: synthesis.keyRisk, color: '#f87171', bg: '#2d0707', border2: '#7f1d1d' },
                    { label: 'Key Opportunity', value: synthesis.keyOpportunity, color: '#4ade80', bg: '#052e16', border2: '#166534' },
                    { label: 'Core Disagreement', value: synthesis.coreDisagreement, color: '#a78bfa', bg: '#1a0d2e', border2: '#4c1d95' }
                  ].map(c => (
                    <div key={c.label} style={{ background: c.bg, border: `1px solid ${c.border2}`, borderRadius: 10, padding: '12px 14px' }}>
            ...
            {!round1 && !loading && (
              <div style={{ textAlign: 'center', padding: '5rem 2rem' }}>
                ...
                <div style={{ fontSize: 13, color: textFaint }}>Try: NVIDIA · Apple · Bitcoin · AI infrastructure · Solar energy</div>
              </div>
            )}
          </>
        )}
        ...
        {tab === 'vault' && (
          <div>
            <div style={{ marginBottom: 24 }}>
              <h2 style={{ fontSize: 20, fontWeight: 800, margin: '0 0 4px', color: text, letterSpacing: '-0.5px' }}>Thesis Vault</h2>
              <p style={{ fontSize: 13, color: textMuted, margin: 0 }}>Your permanent research library — every analysis you've run.</p>
            </div>

            {vaultLoading && <div style={{ textAlign: 'center', padding: '3rem', color: textMuted }}>Loading vault...</div>}
            ...
            {!vaultLoading && vault.length === 0 && (
              <div style={{ textAlign: 'center', padding: '5rem', color: textFaint }}>
                ...
                <div style={{ fontSize: 13, color: textFaint, marginBottom: 20 }}>Run your first analysis to start building your research library</div>
            ...
            {!vaultLoading && vault.length > 0 && !selectedAnalysis && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {vault.map(a => {
                  ...
                  const vBorder = a.verdict === 'Bullish' ? '#166534' : a.verdict === 'Bearish' ? '#7f1d1d' : '#1e293b';
                  return (
                    <div key={a.id} onClick={() => setSelectedAnalysis(a)}
                      ...
                      <div style={{ flex: 1, minWidth: 200 }}>
                        <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 3, color: text }}>{a.company_name || a.query}</div>
                      ...
                      <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
                        {a.price_at_analysis && <div style={{ fontSize: 13, color: textMuted, fontWeight: 500 }}>${a.price_at_analysis}</div>}
                        <div style={{ background: vBg, color: vColor, border: `1px solid ${vBorder}`, fontSize: 11, fontWeight: 700, padding: '4px 10px', borderRadius: 6, letterSpacing: '0.3px' }}>{a.verdict}</div>
                        <div style={{ fontSize: 11, color: textMuted }}>{a.confidence}% confidence</div>
                      </div>
                    </div>
                  );
                })}
            ...
            {selectedAnalysis && (
              <div>
                <button onClick={() => setSelectedAnalysis(null)} style={{ fontSize: 13, color: textMuted, background: 'none', border: 'none', cursor: 'pointer', marginBottom: 20, padding: 0 }}>← Back to vault</button>
                <div style={{ background: surface, border: `1px solid ${border}`, borderRadius: 14, padding: '1.5rem', marginBottom: 16 }}>
                  <div style={{ fontSize: 22, fontWeight: 800, marginBottom: 4, color: text, letterSpacing: '-0.5px' }}>{selectedAnalysis.company_name || selectedAnalysis.query}</div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 8 }}>
                    {[
                      ...
                      <div key={c.label} style={{ background: surface2, border: `1px solid ${border}`, borderRadius: 8, padding: '10px 12px' }}>
                        <div style={{ fontSize: 9, color: textFaint, textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 4 }}>{c.label}</div>
                        <div style={{ fontSize: 15, fontWeight: 700, color: c.color || text }}>{c.value}</div>
                      </div>
                    ))}
                  </div>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 10, marginBottom: 16 }}>
                  {[
                    { label: 'Key Risk', value: selectedAnalysis.key_risk, color: '#f87171', bg: '#2d0707', border2: '#7f1d1d' },
                    { label: 'Key Opportunity', value: selectedAnalysis.key_opportunity, color: '#4ade80', bg: '#052e16', border2: '#166534' },
                    { label: 'Core Disagreement', value: selectedAnalysis.core_disagreement, color: '#a78bfa', bg: '#1a0d2e', border2: '#4c1d95' }
                  ].map(c => (
                    <div key={c.label} style={{ background: c.bg, border: `1px solid ${c.border2}`, borderRadius: 10, padding: '12px 14px' }}>
                      <div style={{ fontSize: 9, color: c.color, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 6, opacity: 0.8 }}>{c.label}</div>
                      <div style={{ fontSize: 13, fontWeight: 600, color: c.color, lineHeight: 1.4 }}>{c.value}</div>
